...
def group_and_save(features, district_name, tehsil_name, custom_output_folder=None):
    ...
    for mouza, feats in grouped.items():
        ...
        # Additionally group features by their computed group key and save each set
        group_folder = os.path.join(tehsil_folder, mouza)
        os.makedirs(group_folder, exist_ok=True)
        key_groups = {}
        for feat in feats:
            key = get_group_key(feat.get("properties", {}))
            if key and key != "0":
                key_groups.setdefault(key, []).append(feat)
        for key, key_feats in key_groups.items():
            key_name = sanitize_filename(key.replace('/', '_'))
            key_path = os.path.join(group_folder, f"{key_name}.geojson")
        ...
        if union_features:
            unified_path = os.path.join(group_folder, f"{mouza}.geojson")
